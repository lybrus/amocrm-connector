...
const amocrm = new AmoCRM({
    ...
    options: {
        ...
        store,
        // Автоматически обновлять токен при запросе
        // Не используйте данный параметр если запускаете больше одного инстанса вашего сервера
        ...
        // Не используйте данный параметр если запускаете больше одного инстанса вашего сервера, контролируйте из отдельной cron job
...
Ответ возвращается для всех 2xx статусов и статуса 304 (not modified). В остальных случаях будет проброшено исключение типа
